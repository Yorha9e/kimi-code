...
// Relative to brandHomeDir, which already IS the brand data dir (~/.omkc or
// $OMKC_HOME/$KIMI_CODE_HOME) — no '.omkc' segment here, or it would nest twice.
...
export interface SkillPathContext {
  ...
  /**
   * Brand data dir — `OMKC_HOME`/`KIMI_CODE_HOME`, or `<userHomeDir>/.omkc` by default.
   * User brand skills live directly under here as `skills/`, so this path
   * carries no `.omkc` segment of its own (that would double the prefix).
   */
...
export async function resolveSkillRoots(
  ...
  const brandHomeDir = options.paths.brandHomeDir ?? path.join(userHomeDir, '.omkc');
