...
- **Never assume the data directory is `~/.omkc`.** Always resolve `$OMKC_HOME` (or legacy `$KIMI_CODE_HOME`) first with the Bash command below.
...
The kimi-code runtime resolves the data directory as `OMKC_HOME` first, then `KIMI_CODE_HOME` (legacy compat), falling back to `~/.omkc`. Theme files live inside the `themes/` subdirectory of that data directory.

Before doing anything, resolve the actual data root with Bash so you don't write to the wrong place. Check whether `OMKC_HOME`/`KIMI_CODE_HOME` is set and fall back to `~/.omkc` when both are empty:
...
Use the first non-empty line; otherwise use the last one. In the rest of this skill, `<KIMI_CODE_HOME>` means that resolved data root — **never assume `~/.omkc`**. Theme files live at `<KIMI_CODE_HOME>/themes/<name>.json`. Create the `themes/` directory if it doesn't exist.
...
2. **Resolve the actual theme directory and current theme(s).**
   - Resolve the data root by checking `echo "$OMKC_HOME"` then `echo "$KIMI_CODE_HOME"`; if both are empty, use `echo "$HOME/.omkc"`. Use `<root>/themes` for every subsequent step.
